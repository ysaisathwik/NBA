...
def test_login_and_me():
    ...
    h = auth(c, "121103ysaisathwik@gmail.com", "Manager#2024")
...
def test_guardrail_rejects_non_problem_and_short_input():
    c = client()
    h = auth(c, "harshithapembarthi953@gmail.com", "Operator#2024")
...
def test_customer_cannot_trigger_events_or_list_sessions():
    c = client()
    h = auth(c, "mtejomurtula@gmail.com", "Customer#2024")
...
def test_operator_cannot_approve_p1_but_manager_can():
    c = client()
    op = auth(c, "harshithapembarthi953@gmail.com", "Operator#2024")
    mgr = auth(c, "121103ysaisathwik@gmail.com", "Manager#2024")
    ...
    assert any("email" in s["title"].lower() for s in final["trace"])  # HTML emails dispatched
    assert final.get("csat_score") is not None


def test_work_update_role_enforcement():
    c = client()
    cust = auth(c, "mtejomurtula@gmail.com", "Customer#2024")
    mgr = auth(c, "121103ysaisathwik@gmail.com", "Manager#2024")
    op = auth(c, "harshithapembarthi953@gmail.com", "Operator#2024")
...
def test_billing_passes_all_three_gates_no_auto_close():
    ...
    op = auth(c, "harshithapembarthi953@gmail.com", "Operator#2024")
    cust = auth(c, "mtejomurtula@gmail.com", "Customer#2024")
    event = {"type": "billing_dispute", "source_type": "crm", "customer_id": "mtejomurtula@gmail.com",
             "severity": "INFO",
